...
void layoutTree::makeLayoutTree(treeNode *node, layoutNode *parentLayout)
{
    ...
    {
        std::vector<std::string> suffix = {"-top", "-bottom", "-right",
                                           "-left"};
        for (int i = 0; i < 4; i++)
    ...
    currentLayoutNode->display = returnDisplayType(
        node->style[node->cssPropertyIndexCache["display"]].value);
    ...
    // now we have to recurse and add layout nodes for all children of the
    // treenode which will be linked to current layout node
    ...
    // this way children do not get added to the wrong container node and tree
    // structure for inline elements is preserved
    currentContainerNode = temp;

    // add it to children of parent layout node if block or parent is inline
    // or add to the last container node if inline. if last node doesnt exist
    // make a new one
    ...
    else if (currentLayoutNode->display == displayType::displayInline)
    {
        ...
        // add to container node only if parent is block
        if (parentLayout->display == displayType::displayBlock)
        {
...
void layoutTree::traverse(layoutNode *node, int level)
{
    ...
    {
        std::cout << indent << "container node ";
    }
    else if (node->type == nodeType::lineContainer)
    {
...
float layoutTree::calculateLayoutInlineContainer(layoutNode *node,
                                                 float availableWidth)
{

    // work on all the children elements and recurse only for elements that may
    // have children text cannot have children so divide it up into more text
    // nodes and put them in line containers inline container node will have no
    // margin or padding because that will be handled by its parent node
    ...
    for (auto child : node->children)
    {
        ...
        default: /* std::cout << "Default node detected in container node" <<
                    std::endl*/
            ;
        }
    }

    // deleting all the old children
    for (auto child : node->children)
        delete child;
...
float layoutTree::calculateLayoutLineContainer(layoutNode *node,
                                               float availableWidth)
{
    float nodeHeight = 0;
    float descender = 0;
    ...
    for (auto child : node->children)
    {
        ...
        if (tempAscender > nodeHeight)
            nodeHeight = tempAscender;
        if (tempDescender > nodeHeight)
            descender = tempDescender;
    }
...
float layoutTree::calculateLayoutText(layoutNode *node, float availableWidth)
{
    ...
    float lineHeight = 0;
    if (node->parent)
        lineHeight = node->parent->height;
    else
        std::cout << "Text node isnt inside a line" << std::endl;
...
void layoutTree::seperateLineText(layoutNode *node, layoutNode *child,
                                  float availableWidth,
                                  std::vector<layoutNode *> &lineContainers)
{
    ...
    for (auto c : child->text)
    {
        if (c == ' ')
        {
            // do the checks and if we hit the max word length make a new line
            // container
            word += c;
...
Color layoutTree::convertStringToColor(std::string &input)
{
    ...
    if (stringToColorMap.count(temp) == 1)
        value = stringToColorMap[temp];
    else
    {
